...
class SupervisorAgent:
    ...
    async def run_orchestration_stream(
        ...
        yield emit_log("FlightAgent", f"Searching available flights from {origin or 'origin'} to {destination}…")
        await asyncio.sleep(0.05)

        yield emit_log("HotelAgent", f"Searching accommodations in {destination}…")
        await asyncio.sleep(0.05)

        yield emit_log("WeatherAgent", f"Fetching weather and climate data for {destination}…")
        await asyncio.sleep(0.05)

        yield emit_log("PlacesAgent", f"Searching top-rated attractions, restaurants, and shopping spots in {destination}…")
        await asyncio.sleep(0.05)

        yield emit_log("BudgetAgent", f"Analysing cost estimates against budget ({budget or 'flexible'})…")
        await asyncio.sleep(0.05)
        ...
        itinerary_text = planner_res.get("itinerary", "") or planner_res.get("narrative", "")
