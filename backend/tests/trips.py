...
def create_trip(
    ...
    return TripRepository(db).create(user.id, payload.destination.strip(), origin=origin)
...
def save_trip(
    trip_id: uuid.UUID,
    payload: TripSaveRequest | None = None,
    user: CurrentUser = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    trip = _owned_trip(trip_id, user, db)
    # Honor an explicit target state when the client sends one (idempotent —
    # matches the client's optimistic UI state). Fall back to toggling only
    # when no body (or an empty body) is sent.
    if payload is not None and payload.saved is not None:
